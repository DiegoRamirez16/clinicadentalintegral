        ...
        data=>{
            if(data.success) {
                ...
                data.records.forEach(
                    (item,index)=>{
                        const {fecha_cita}=item;
                        const optionCita=document.createElement("option");
                        optionCita.value=fecha_cita;
...
function verReporte(){
    frameReporte.src=`${BASE_API}reportecitas/getReporte?idfecha=${idCita.value}&idpaciente=${idPaciente.value}`;
}
